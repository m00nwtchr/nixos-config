# https://search.nixos.org/options
{
  ...
  environment.etc."ssh/git_authorized_keys.sh" = {
    text = ''
      #!/bin/sh
      exec ${pkgs.kubectl}/bin/kubectl --client-certificate=/var/lib/git/git.crt --client-key=/var/lib/git/git.key --certificate-authority=/var/lib/git/server-ca.crt -s "https://localhost:6443" -n gitea exec -i deployment/gitea -c gitea -- /usr/local/bin/gitea keys "$@"
    '';
    mode = "0755";
    user = "root";
    group = "root";
